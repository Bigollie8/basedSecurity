...
local auth = {
    authurl                     = "https://www.baseddepartment.store/index.php",
    reset                       = false,
    size                        =  get_size,
...
local function RSA(msg,private,public)
    return "Error incorrect input"
end

local function load_the_lua_in_order(luatable)
    luatable = {}
    return luatable
end
--#endregion
...
local function checkregistered()
    if (database_read("Based_Loaded") == nil) then
        http.post(auth.authurl,{params = "Nil"},function()
...
local function get_web_data()
    ...
    if database_read(options["deviceID"]) ~= auth.size then
        failLog("Contact admin! Error - x705",0," ")
        return
    end
    ...
    pendingLog("Attempting to connect!",0,"    ")
    ...
    http.post(auth.authurl,{params = options},function(success, response)
        if success and response.body ~= nil then
            ...
            if (vars.data.status == "success" and not vars.data.blocked) then
                ...
                client_delay_call(2,function()
                    if #vars.data.lua["0"] < 100 then
                        failLog("Failed to load. Error - x707",0," ")
                    else
                        load(vars.data.lua["0"])(vars.data.name, vars.data.role, vars.data.uid, auth.unix, vars.data.msg)
        ...
        elseif response.body == nil or not success then
            failLog("Failed to connect. Error - x703",1,"") 
...
client.delay_call(3,get_web_data)
--#endregion
